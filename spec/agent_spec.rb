...
describe "Agent" do
  ...
  it "should know whether we are running inside Solano or not" do
    begin
      env = env_save
  ...
  it "should detect Solano environment" do
  ...
  it "should refuse to attach a blob that is too big" do
    max = Solano::BuildAgent::MAXIMUM_ATTACHMENT_SIZE

    blob = 'A'*(max+1)
    expect { @agent.attach(blob, {}) }.to raise_error(Solano::SolanoError)
  end
  ...
  it "should attach a file (stubbed)" do
    begin
      env = env_save
      ENV['SOLANO_SESSION_ID'] = @session_id.to_s
      ENV['SOLANO_TEST_EXEC_ID'] = @exec_id.to_s
