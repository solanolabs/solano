...
Feature: Account command
  As a Solano user
  In order to use Solano in my organization
...
Scenario: Fail if the user is logged into multiple accounts
  ...
  When I run `solano account`
  Then it should fail with "Your .solano file has an invalid API key."

Scenario: Display account info if the .solano files match
...
Scenario: Add member to account successfully
  ...
  When I successfully run `solano account:add member member2@example.com`
...
Scenario: Add member to account unsuccessfully
  ...
  When I run `solano account:add member member@example.com`
...
Scenario: Remove member from account successfully
  ...
  When I successfully run `solano account:remove member@example.com`
...
Scenario: Remove member from account unsuccessfully
  ...
  When I run `solano account:remove member@example.com`
