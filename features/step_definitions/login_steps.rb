...
Given /^the user is logged in to multiple accounts$/ do
  ...
  steps %Q{
    ...
    {"api_key":"abcdef"}
    """
    And a solano global config file exists with:
    """
    {"api_key":"hijklm"}
    """
  }
...
Given /^the user has a .solano for branch "(.*)"$/ do |branch|
  steps %Q{
    Given a file named ".solano.localhost" with:
    """
...
Given /^the user is logged in with a configured suite(?: on branch "(.*)")?$/ do |branch|
  ...
  Antilles.install(:get, "/1/accounts/usage_by_account", SAMPLE_ACCOUNT_USAGE)
  steps %Q{
    Given the user has a .solano for branch "#{branch}"
...
Given /^a solano global config file exists(?: with:)$/ do |content|
...
Then /^the solano global config file should not exist$/ do
  File.should_not exist(solano_global_config_file_path)
end

Then /^dotfiles should be updated$/ do
  steps %Q{
    And the file ".solano.localhost" should contain "apikey"
    And the file ".gitignore" should contain ".solano"
    And the file ".gitignore" should contain ".solano*"
  }
end
...
Then /^options should not be saved$/ do
  steps %Q{
    Then the file ".solano.localhost" should not contain "test_pattern"
    And the file ".solano.localhost" should not contain "max_parallelism"
    And the file ".solano.localhost" should not contain "user_data_file"
  }
end
